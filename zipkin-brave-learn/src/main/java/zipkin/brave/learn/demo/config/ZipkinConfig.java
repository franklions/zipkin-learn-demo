package zipkin.brave.learn.demo.config;
...
public class ZipkinConfig {
    ...
    @Bean
    public SpanCollector spanCollector(@Value("${zipkin.base-url:http://localhost:9411}") String baseUrl){
        HttpSpanCollector.Config config = HttpSpanCollector.Config.builder()
                ...
                .readTimeout(6000)
                .build();
        return HttpSpanCollector.create(baseUrl,config,
                new EmptySpanCollectorMetricsHandler());
    }

    //作为各调用链路，只需要负责将指定格式的数据发送给zipkin
    @Bean
    public Brave brave(SpanCollector spanCollector,@Value("${spring.application.name:unknown}") String serviceName){
